...
async function waitForJobCompletion(jobId, maxAttempts = 10) {
  for (let attempt = 0; attempt < maxAttempts; attempt++) {
    const pollResponse = await request(app).get(`/api/results?jobId=${jobId}`);
    if (pollResponse.status !== 200) {
      return pollResponse;
    }
    if (pollResponse.body.status !== "processing") {
      return pollResponse;
    }
  }
  return request(app).get(`/api/results?jobId=${jobId}`);
}
...
describe("POST /api/process", () => {
  ...
  test("returns frontend-shaped data response", async () => {
    ...
    const completed = await waitForJobCompletion(response.body.jobId);
  ...
  test("supports polling result by jobId via /api/results?jobId=...", async () => {
    ...
    const pollResponse = await waitForJobCompletion(processResponse.body.jobId);
  ...
  test("returns 500 when java execution fails", async () => {
    ...
    expect(pollResponse.body.error).toMatch(/simulated stderr/i);
  });
...
describe("GET /api/download/:jobId", () => {
  ...
  test("downloads CSV for a known completed job", async () => {
    const csvText = [
      "timestamp,x,y",
      "0.000,200,436",
      "1.000,-1,-1",
  ...
  test("returns 404 for unknown jobId", async () => {
    const response = await request(app)
      .get("/api/download/job-does-not-exist");
  ...
  test("returns 404 when CSV path is missing on disk", async () => {
    const csvText = [
      "timestamp,x,y",
      "0.000,200,436",
    ].join("\n");
    ...
    const response = await request(app).get(`/api/download/${processResponse.body.jobId}`);

    expect(response.status).toBe(404);
    expect(response.body.success).toBe(false);
    expect(response.body.error).toMatch(/CSV not found/i);
  });
});
